                                        ...
                                        @foreach($cart as $carts)
                                          <li id="{{$carts->id}}">{{$carts->product()->first()->name}} <br><strong class="pull-right text-primary" >
                                            ...
                                            <li><b> Cart Subtotal</b><strong class="pull-right text-primary"  id="cart_subtotal" data-subtotal = "{{$total }}">{{$total }} EGP</strong></li>
                                             <li><b> Shipping Cost</b><strong class="pull-right text-primary" id="shipping_price" data-shippingPrice = "0">@guest 0.00 EGP @else {{auth()->user()->city->city_shippingPrice}} @endguest</strong></li>
      ...
      var value     = $(this).val(),
        ...
        $.ajax({
          ...
          success:function(result){
            ...
            {
              $('#cart_total').html(+$('#cart_total').data("subtotal") - +$('#subtotal').data("total") * +result.coupon.code_value /100 +" EGP");  
              $("#discount_rate").show();
      ...
      var value     = f.originalEvent.clipboardData.getData('text'),
        ...
        $.ajax({
          ...
          success:function(result){
            if(result.status == true)
            {
              $('#cart_total').html(+$('#cart_total').data("subtotal") - +$('#cart_total').data("subtotal") * +result.coupon.code_value /100 +" EGP");  
              $("#discount_rate").show();
              $("#discount_value").text(result.coupon.code_value+"%");
              $("#coupon_error").html(` <strong style="color:#f24e97">${result.success}</strong>`);
              $('#code').prop("disabled", false);
